...
var AUTOFILL_SOURCES = [
  {
    ...
    生ログ種別: 'raw_switchbot',
    必要フラグ: '服薬自動',
    説明: 'SwitchBot服薬ログ → 「服薬ボックス開放」（服薬確認の質問に添える補助情報）',
    map: function (raw) {
      // 実際の運用では、このログを見た夜勤者が声かけをしている。
      // つまりセンサーは「支援の記録」ではなく「支援を起こす合図」なので、
      // 服薬確認そのものは自動で埋めず、開放の事実（時刻があれば時刻も）だけを残す。
      var v = String(raw['値'] || '').trim();
      return [{ 項目名: '服薬ボックス開放', 値: v ? ('開放を検知 ' + v + '（SwitchBot自動記録）') : '開放を検知（SwitchBot自動記録）' }];
    }
  },
  {
    id: 'door_sensor',
