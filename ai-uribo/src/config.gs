...
var SHEET_DEFS = [
  ...
  {
    name: SHEETS.FILL,
    headers: ['fill_id', '対象日', '対象', '項目名', '値', '記入者staff_id', '取込済フラグ', '作成日時', 'gap_id', '情報源'],
    note: '補完台帳。既存アプリはここを読んで自分のDBに取り込む。gap_idは二重記録の防止に使う。'
        + '情報源＝本人回答/支援担当者の記録/支援担当者名＋管理者確認/自動ログ（監査時に「誰の情報か」を示すため）'
  },
  {
    name: SHEETS.SETTING,
    headers: ['キー', '値', '説明'],
...
var INITIAL_CHECKS = [
  ...
  ['CHK106', 'support', '服薬確認', 'R02', '夜勤', 'A',
   ...
   '声かけ・確認をした|本人が自分で服薬|実施なし（一言記入）|わからない', false],
