...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('AI Uribo')
    ...
    .addSeparator()
    .addItem('シフト希望のリハーサル（今すぐ1回だけ）', 'menuRehearseShift_')
    .addItem('★ 本番運用を開始する', 'menuGoLive_')
    .addItem('テストモードに戻す', 'menuGoTest_')
    .addItem('SwitchBotをつなぐ', 'menuSetSwitchbot_')
    .addSeparator()
 ...
 *   ・終わったら試験前の状態に戻す（本番運用中なら本番運用中のまま）
 *
 * 2026-09-21、本番運用中に確認のつもりで実行したところ、終わってもテストモードの
 * ままになり、気づかず本番のLINE送信が止まっていた。「勝手に本番へ戻さない」という
 * 判断は逆で、**試験は試験前の状態に戻すのが自然**。以後はここを自動で戻す。
 */
...
function selfTestBody_(proc) {
  ...
  // 試験前の状態に戻す。本番運用中に「確認のつもりで」実行しても、本番を止めない
  setSetting_('test_mode', beforeMode);
  ...
  lines.push('');
  lines.push(isTrue_(beforeMode)
    ? 'テストモードはONのままです（試験前からONでした）'
    : 'テストモードを試験前の状態（本番運用中）に戻しました');
  lines.push('===== ここまで =====');
